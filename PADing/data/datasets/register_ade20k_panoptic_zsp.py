...
_PREDEFINED_SPLITS_ADE20K_PANOPTIC = {
    "ade20k_train_panoptic": (
        "ADEChallengeData2016/images/training",  # image_root
        # Mask2former生成
        "ADEChallengeData2016/ade20k_panoptic_train",  # panoptic_root
        "ADEChallengeData2016/annotations/ade20k_panoptic_train.json",  # panoptic_json
        "ADEChallengeData2016/annotations_detectron2/training",  # semantic_root
        "ADEChallengeData2016/ade20k_instance_train.json",  # instance_json
    ),
    "ade20k_val_panoptic": (
        "ADEChallengeData2016/images/validation",
        "ADEChallengeData2016/ade20k_panoptic_val",
        "ADEChallengeData2016/annotations/ade20k_panoptic_val.json",
...
def load_ade20k_panoptic_json(json_file, image_dir, gt_dir, meta):
    """
    Args:
...
def load_ade20k_panoptic_json_semantic(json_file, image_dir, gt_dir, semseg_dir, meta):
    """
    Args:
        image_dir (str): path to the raw dataset. e.g., "~/coco/train2017".
        gt_dir (str): path to the raw annotations. e.g., "~/coco/panoptic_train2017".
    ...
    # annotations = json_info["annotations"][:10] if DEBUG else json_info["annotations"]
...
def register_ade20k_panoptic_seen(
        name, metadata, image_root, panoptic_root, panoptic_json, instances_json=None
):
    panoptic_name = name
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_ade20k_panoptic_json(panoptic_json, image_root, panoptic_root, metadata),
    )
    ...
    MetadataCatalog.get(panoptic_name).set(
        ...
        ignore_label=255,
        label_divisor=1000,
        evaluation_set=ADE20K_UNSEEN_CLASSES,
        unseen_index=NOVEL_ID,
        **metadata,
    )


def register_all_ade20k_panoptic_seen(root):
    prefix = "ade20k_train_panoptic_zsi"
    panoptic_root = "ADEChallengeData2016/ade20k_panoptic_train"
    panoptic_json = "ADEChallengeData2016/annotations/ZSP/panoptic_train_seen.json"

    prefix_instances = prefix[: -len("_zsi")]
    instances_meta = MetadataCatalog.get(prefix_instances)
    image_root, instances_json = instances_meta.image_root, instances_meta.json_file

    register_ade20k_panoptic_seen(
        prefix,
        get_metadata(),
        image_root,
        os.path.join(root, panoptic_root),
...
def register_ade20k_panoptic_annos_sem_seg(
    ...
    DatasetCatalog.register(
        semantic_name,
        lambda: load_ade20k_panoptic_json_semantic(panoptic_json, image_root, panoptic_root, sem_seg_root, metadata),
    )
    ...
    MetadataCatalog.get(semantic_name).set(
        ...
        evaluator_type="ade20k_panoptic_seg",
        ignore_label=255,
        val_extra_classes=ADE20K_UNSEEN_CLASSES,
        label_divisor=1000,
        evaluation_set=ADE20K_UNSEEN_CLASSES,
        unseen_index=NOVEL_ID,
        **metadata,
...
def register_all_ade20k_panoptic_annos_sem_seg(root):
    prefix = "ade20k_val_panoptic_zsi"
    panoptic_root = "ADEChallengeData2016/ade20k_panoptic_val"
    panoptic_json = "ADEChallengeData2016/annotations/ZSP/panoptic_val_all.json"
    semantic_root = "ADEChallengeData2016/panoptic_semseg_val"

    prefix_instances = prefix[: -len("_zsi")]
    instances_meta = MetadataCatalog.get(prefix_instances)
    image_root, instances_json = instances_meta.image_root, instances_meta.json_file

    register_ade20k_panoptic_annos_sem_seg(
        prefix_instances,
        get_metadata(),
        image_root,
        os.path.join(root, panoptic_root),
...
# # 基于上面的注册训练的(仅包含seen)
register_all_ade20k_panoptic_seen(_root)
# # 基于上面的注册验证的(同时包含seen和unseen)
register_all_ade20k_panoptic_annos_sem_seg(_root)
